...
import { PermissionDeniedError, type Persona } from './types.js'
...
describe('enforce', () => {
  it('returns verdict on allow', () => {
    const v = enforce(
      { persona: RESEARCHER, toolName: 'read_file', args: { path: 'a' } },
      { now: T0 },
    )
    expect(v.allowed).toBe(true)
  })
  ...
  it('throws PermissionDeniedError on deny', () => {
    expect(() =>
      enforce({ persona: RESEARCHER, toolName: 'write_file', args: {} }, { now: T0 }),
