...
interface RateBucket {
  /** timestamps of recent invocations within the window */
  hits: number[]
}

/** keyed by `${persona.id}::${toolName}` */
const RATE_LIMIT_STATE: Map<string, RateBucket> = new Map()
...
function rateKey(personaId: string, toolName: string): string {
  return `${personaId}::${toolName}`
}

function blastRadiusRank(r: BlastRadius): number {
  return BLAST_RADIUS_ORDER.indexOf(r)
}

function maxBlastRadius(a: BlastRadius, b: BlastRadius): BlastRadius {
  return blastRadiusRank(a) >= blastRadiusRank(b) ? a : b
}

function patternMatches(pattern: string | RegExp, toolName: string): boolean {
...
function checkArgRule(
  ...
  if (rule.type === 'string') {
    if (typeof value !== 'string') return { ok: false, reason: `arg "${argName}" must be string` }
    if (rule.pattern) {
  ...
  if (rule.type === 'boolean') {
    if (typeof value !== 'boolean') return { ok: false, reason: `arg "${argName}" must be boolean` }
    if (rule.allowedValues && !rule.allowedValues.includes(value)) {
      return { ok: false, reason: `arg "${argName}" not in allowedValues` }
    }
    return { ok: true }
...
function checkArgs(
  ...
  if (!scope.argConstraints) return { ok: true }
...
function checkRateLimit(
  ...
  if (!scope.rateLimit) return { ok: true }
...
export function canInvoke(
  ...
  let lastFailure: { reason: string; scope: Scope } | null = null
  let anyToolMatch = false
  const personaCap = persona.maxBlastRadius

  for (const scope of persona.scopes) {
    if (!patternMatches(scope.toolPattern, toolName)) continue
    anyToolMatch = true

    // enforce blast-radius cap
    const scopeRadius: BlastRadius = scope.blastRadius ?? personaCap ?? 'none'
 ...
 */
export function enforce(grant: PermissionGrant, opts?: { now?: number }): Verdict {
  const verdict = canInvoke(grant.persona, grant.toolName, grant.args, opts)
  if (!verdict.allowed) throw new PermissionDeniedError(grant, verdict)
  return verdict
}
 ...
 */
export function mergePersonas(...personas: Persona[]): Persona {
  ...
  const scopes: Scope[] = personas.flatMap((p) => p.scopes)
  let cap: BlastRadius = 'none'
  for (const p of personas) {
    if (p.maxBlastRadius) cap = maxBlastRadius(cap, p.maxBlastRadius)
  }
 ...
 */
export function loadPersona(id: string, registry: Record<string, Persona>): Persona {
