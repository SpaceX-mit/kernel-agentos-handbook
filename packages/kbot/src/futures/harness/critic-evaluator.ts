...
export interface CriticEvaluatorOpts {
  ...
  gateFn?: (
    ...
    result: unknown,
  ) => Promise<CriticVerdict> | CriticVerdict
}

/** Map RF taxonomy classes onto harness FailureMode kinds. */
function rfToFailureKind(rf: RFClass): FailureModeKind {
  switch (rf) {
...
function flattenTrace(trace: ExecutionTrace): string {
  const parts: string[] = []
...
export class CriticEvaluator implements Evaluator {
  constructor(private readonly opts: CriticEvaluatorOpts = {}) {}

  async evaluate(trace: ExecutionTrace, task: Task): Promise<EvaluationReport> {
    const haystack = flattenTrace(trace).toLowerCase()
    const criteriaResults: CriterionResult[] = task.acceptance.map((criterion) => {
...
export function createCriticEvaluator(opts: CriticEvaluatorOpts = {}): Evaluator {
