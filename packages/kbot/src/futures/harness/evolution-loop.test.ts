...
function mkHarness(id: string): Harness {
  return {
    id,
...
class CannedWorker implements Worker {
  private i = 0
  constructor(private outputs: string[]) {}
  async execute(task: Task, harness: Harness): Promise<ExecutionTrace> {
...
class RegexEvaluator implements Evaluator {
  constructor(private pattern: RegExp) {}
  async evaluate(trace: ExecutionTrace, task: Task) {
...
describe('runEvolutionLoop', () => {
  it('runs K=3 iterations and reports history length', async () => {
    const protocol: EvolutionProtocol = {
  ...
  it('persists records via injected hook', async () => {
    const captured: EvolutionRecord[] = []
    const protocol: EvolutionProtocol = {
