...
function compareVerdict(prev: number, current: number, revertThreshold?: number): EvolutionRecord['verdict'] {
  if (current > prev) return 'improved'
  if (current === prev) return 'no-op'
...
export async function runEvolutionLoop(
  ...
  let bestHarness: Harness = initialHarness
