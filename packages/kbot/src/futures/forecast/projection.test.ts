...
function buildLinear(name: string, days: number, intercept: number, slopePerDay: number): Signal {
...
function buildExponential(name: string, days: number, base: number, ratePerDay: number): Signal {
  const values: Array<{ ts: number; value: number }> = []
  for (let i = 0; i < days; i++) {
...
function buildFlatNoisy(name: string, days: number, mean: number, seed = 1): Signal {
...
describe('linearProjection', () => {
  ...
  it('falls back to flat when fewer than 2 points', () => {
    const sig: Signal = { name: 'one-point', values: [{ ts: T0, value: 42 }] }
    const f = linearProjection(sig, '7d')
