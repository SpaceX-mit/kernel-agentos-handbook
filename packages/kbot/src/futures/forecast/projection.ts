...
import { type Forecast, type Horizon, type Signal, type Trend, HORIZON_MS } from './types.js'
 ...
 */
function leastSquares(xs: number[], ys: number[]): FitResult {
  const n = xs.length
  let sumX = 0
  let sumY = 0
  for (let i = 0; i < n; i++) {
...
function sortedValues(signal: Signal): Array<{ ts: number; value: number }> {
  return [...signal.values].sort((a, b) => a.ts - b.ts)
}

/** Map r² + sample size to a 0..1 confidence score. */
function confidenceFrom(r2: number, n: number): number {
  if (n < 2) return 0
...
function flatForecast(signal: Signal, horizon: Horizon, method = 'flat-mean'): Forecast {
  ...
  const trend: Trend = { kind: 'flat', slope: 0, r2: 0 }
  return {
    signal: signal.name,
    horizon,
    trend,
 ...
 */
export function linearProjection(signal: Signal, horizon: Horizon): Forecast {
  const pts = sortedValues(signal)
  if (pts.length < 2) return flatForecast(signal, horizon, 'flat-too-few')
  ...
  const trend: Trend = { kind: 'linear', slope: fit.slope, r2: fit.r2 }
  return {
    signal: signal.name,
    horizon,
    trend,
 ...
 */
export function exponentialProjection(signal: Signal, horizon: Horizon): Forecast {
  const pts = sortedValues(signal).filter((p) => p.value > 0)
  if (pts.length < 2) return flatForecast(signal, horizon, 'flat-nonpositive')
  ...
  const trend: Trend = { kind: 'exponential', slope: fit.slope, r2: fit.r2 }
  return {
    signal: signal.name,
    horizon,
    trend,
...
export function flatProjection(signal: Signal, horizon: Horizon): Forecast {
 ...
 */
export function bestProjection(signal: Signal, horizon: Horizon): Forecast {
  const pts = sortedValues(signal)
  if (pts.length < 2) return flatProjection(signal, horizon)
 ...
 */
export function clampHorizon(h: Horizon, history: number): boolean {
  if (history <= 0) return false
  return HORIZON_MS[h] <= history * 3
}

/** Helper for tests/synthesize: compute the timespan of a Signal in ms. */
export function signalHistory(signal: Signal): number {
  if (signal.values.length < 2) return 0
