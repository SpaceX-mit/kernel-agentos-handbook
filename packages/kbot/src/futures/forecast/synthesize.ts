 ...
 * surface first.
 */
export function synthesizeForecasts(signals: Signal[], horizon: Horizon): Forecast[] {
  const out: Forecast[] = []
  for (const sig of signals) {
    if (!clampHorizon(horizon, signalHistory(sig))) continue
...
function formatNumber(n: number): string {
  ...
  if (abs >= 1_000) return `${(n / 1_000).toFixed(1)}k`.replace('.0k', 'k')
...
function arrowFor(slope: number, kind: Forecast['trend']['kind']): string {
  if (kind === 'flat') return ARROW.flat
  if (slope > 0) return ARROW.up
  if (slope < 0) return ARROW.down
  return ARROW.flat
}
 ...
 */
export function narrative(forecasts: Forecast[]): string {
