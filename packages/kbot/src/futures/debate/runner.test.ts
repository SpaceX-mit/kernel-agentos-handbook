...
class StubClient implements LLMClient {
  public calls: Array<{ role: AsymmetricRoles; prompt: string }> = [];
  private counters: Record<AsymmetricRoles, number> = {
    'allow-advocate': 0,
    'block-advocate': 0,
    judge: 0,
  };
  ...
  constructor(
    private readonly canned: Record<AsymmetricRoles, string[]>,
  ) {}

  async respond(prompt: string, role: AsymmetricRoles): Promise<string> {
...
function makeStub(judgeOutput: string): StubClient {
...
describe('formatPrompt', () => {
  it('includes context and category hint when provided', () => {
    const p = formatPrompt(
      { candidate: 'X', context: 'CTX', categoryHint: 'cat' },
      'allow-advocate',
      [],
    );
