 ...
 */
export function classifyOutcome(ctx: ToolContext): 'success' | 'error' | 'timeout' | 'empty' | 'large' {
  ...
  if (ctx.error || ctx.aborted) return 'error'
  const result = ctx.result ?? ''
  if (result.length < OUTCOME_EMPTY_THRESHOLD) return 'empty'
  if (Buffer.byteLength(result, 'utf8') > OUTCOME_LARGE_THRESHOLD) return 'large'
  return 'success'
}
...
export function observerMiddleware(
  ...
  return async (ctx, next) => {
    ...
    try {
      ...
      threw = err
      if (!ctx.error) ctx.error = err instanceof Error ? err.message : String(err)
    }
    ...
    try {
      ...
      const a = (ctx.toolArgs ?? {}) as Record<string, unknown>
      const args: Record<string, unknown> = {}
      if (typeof a.file_path === 'string') args.file_path = a.file_path
      else if (typeof a.path === 'string') args.path = a.path
      if (typeof a.command === 'string') args.command = (a.command as string).slice(0, 200)
      if (typeof a.pattern === 'string') args.pattern = a.pattern
      if (typeof a.url === 'string') args.url = a.url
      if (typeof a.query === 'string') args.query = a.query
